...
def careplan_decider(age, sex, chat, careplan, scratchpad, discussion):
    task = Task(
        description=f"""
---
...
---
Given the instructions, this is the medical dialogue you see for a {age} {sex} patient:
---
Medical Dialogue
---
{chat}
...
presented with compelling arguments by the verifying care plan writer, you add to the correction scratchpad.
...
{discussion}
---
Question: What do you say next? Respond to Person B in the tag [RESPONSE: "<your_response_here>"] and output
any corrections to add to the scratchpad in the tag [SCRATCHPAD: "<things_to_add_to_the_scratchpad_here>"]. Make sure to use
the "[]" when outputting tags. All text should be within the tag brackets. An example answer would be:
[RESPONSE: "I think we should remove ... from the care
plan"] [SCRATCHPAD: "Remove ... from the care plan because ..."]
---
Answer:
""",
        expected_output="output the next response",
        agent=decider,
    )
