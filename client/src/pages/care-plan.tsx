...
export function CarePlan() {
  ...
  async function getSummary() {
    const convo = await fetch(`${BASE_URL}/get-care-plan/${convoId}`);
    const data = await convo.json();
  ...
  async function loadHtml() {
    setHtml(
      await marked(
        careplan.careplan.replace("Corrected Summary", "").replace("[STOP]", "")
      )
    );
  }
      ...
      <>
        <main className="flex flex-col justify-start items-start h-full">
          ...
          <div className="px-5 py-4 flex flex-col items-center justify-center w-full">
            <h3 className="mb-5">HealthAI Care Plan Generation</h3>
            <img className="w-10" src="/spinner.gif" alt="" />
            <p className="text-center">
              Please wait, care plan generation usually takes 3-4 minutes
            </p>
          </div>
        </main>
        {/* <p>f: {log}</p> */}
      ...
      <div className="px-5">
        ...
        <div className="flex items-center justify-start gap-2">
          <p className="text-sm italic text-zinc-600">
            Last updated: {new Date(careplan.created_at).toDateString()}
          </p>
        </div>
        <div
